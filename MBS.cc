...
void MBS::set_alpha(double alpha)
{
    _alpha = alpha;
}

void MBS::set_superpixel_size(int32_t sp_size)
{
    _sp_size = sp_size;
}

int32_t MBS::superpixel_segmentation(cv::Mat& image)
{
    cv::Mat smoothed_image;
    cv::boxFilter(image, smoothed_image, -1, cv::Size(5, 5));

    int32_t w = smoothed_image.cols;
    int32_t h = smoothed_image.rows;
    _imgWidth = w;
    _imgHeight = h;

    // generate grid seeds
    /*	int32_t spSize = (w*h) / _sp_size;*/
    int32_t step = sqrt((double)_sp_size) + 0.5;
    ...
    if (_sp_size > minSpSize) {
        pydLevels = int32_t(0.5 * log((float)minSpSize / _sp_size) / log(scaleRatio)) + 1;
    }

    cv::Mat* pyd = new cv::Mat[pydLevels];
    pyd[0] = smoothed_image.clone();
    ...
    for (int32_t k = pydLevels - 1; k >= 0; k--) {
        int32_t size = _sp_size * pow(scaleRatio, 2 * k);
...
int32_t* MBS::get_superpixel_labels()
{
    return _labels;
}

cv::Mat MBS::get_seeds()
{
    return _seeds;
}

cv::Mat MBS::get_superpixel_elements()
{
...
void MBS::DistanceTransform_MBD(cv::Mat& image,
                                float* seedsX,
                                float* seedsY,
                                int32_t cnt,
                                int32_t* labels,
                ...
                for (int32_t k = 0; k <= 1; k++) {
                    ...
                    if (candix >= 0 && candix < w && candiy >= 0 && candiy < h) {
                        ...
                        if (sIdx >= 0) {
                            ...
                            float cDis = std::max(std::max(colorDis[0], colorDis[1]), colorDis[2]) /
                                         255.;

                            float sDis2 = (seedsX[sIdx] - j) * (seedsX[sIdx] - j) +
                                          (seedsY[sIdx] - i) * (seedsY[sIdx] - i);
...
int32_t MBS::FastMBD(cv::Mat& img,
                     int32_t* labels,
                     int32_t spSize,
                     int32_t outIter,
                     int32_t inIter,
            ...
            if (!visited[pixIdx]) {
                ...
                for (int32_t c = 0; c < count; c++) {
                    for (int32_t n = 0; n < 4; n++) {
                        ...
                        if ((x >= 0 && x < w) && (y >= 0 && y < h)) {
                            int32_t nbIdx = y * w + x;
                            if (!visited[nbIdx] && currLabel == ioLabels[nbIdx]) {
                                xvec[count] = x;
                                yvec[count] = y;
                                visited[nbIdx] = 1;
                                count++;
